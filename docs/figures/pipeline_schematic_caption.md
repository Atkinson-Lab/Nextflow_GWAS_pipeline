...
Both use five phases per pipeline with one line each, no module numbers, no group subsets, one "illustrative" tag on the benchmark insets, and the "This framework" column in bold green. Regenerate (from the genotype pipeline repo) with `python3 make_pipeline_schematic_v2.py --layout lanes|stack|cancer`.
